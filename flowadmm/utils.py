...
from flowadmm.models import UNet
import warnings
import lpips
...
def define_model(args):
    if args.model == "ot" or args.model == "flow_indp":
        ...
        # The released PnP-GS checkpoints use the same UNet backbone shape
        # as the OT model, but without attention blocks.
        model = UNet(input_channels=args.num_channels,
                     input_height=args.dim_image,
                     ch=32,
                     ch_mult=(1, 2, 4, 8),
        ...
        import flowadmm.image_generation.losses as losses
        import flowadmm.image_generation.models.utils as mutils
        import flowadmm.image_generation.models.ema as mema
        from flowadmm.image_generation.configs.rectified_flow.celeba_hq_pytorch_rf_gaussian import get_config as get_config_celebahq
        from flowadmm.image_generation.configs.rectified_flow.afhq_cat_pytorch_rf_gaussian import get_config as get_config_afhq_cat
...
def load_model(name_model, model, state, download=False, checkpoint_path=None, dataset=None, device='cuda'):
    ...
    if name_model == "ot" or name_model == "flow_indp":
        if download:
        ...
        from flowadmm.image_generation.utils import restore_checkpoint
...
def save_images(clean_img, noisy_img, rec_img, args, H_adj, iter='final'):
    raw_noisy_img = noisy_img.clone()
    measurement_img = _display_measurement_image(raw_noisy_img, args)
    motion_kernel = _motion_blur_kernel_for_display(args, clean_img.device)
    ct_fbp_img = _ct_fbp_for_display(raw_noisy_img, args)
    clean_img = _clip_for_display(postprocess(clean_img.clone(), args))
    noisy_img = _clip_for_display(_display_noisy_image(raw_noisy_img, args, H_adj))
    rec_img = _clip_for_display(postprocess(rec_img.clone(), args))
    if measurement_img is not None:
        measurement_img = _clip_for_display(measurement_img)
    if ct_fbp_img is not None:
        ct_fbp_img = _clip_for_display(ct_fbp_img)
    ...
    if measurement_img is not None:
        measurement_img = measurement_img.permute(0, 2, 3, 1).cpu().data
    if ct_fbp_img is not None:
        ct_fbp_img = ct_fbp_img.permute(0, 2, 3, 1).cpu().data
    ...
    if iter == 'final':
        ...
        if measurement_img is not None:
            if batch_size == 1:
                fig = plt.figure()
                plt.imshow(measurement_img[0].numpy())
            elif batch_size == 2:
                fig, ax = plt.subplots(1, 2)
                ax[0].imshow(measurement_img[0].numpy())
                ax[1].imshow(measurement_img[1].numpy())
            else:
                fig, ax = plt.subplots(rows, cols, figsize=(20, 20))
                for i in range(rows):
                    for j in range(cols):
                        if measurement_img.shape[-1] == 1:
                            ax[i, j].imshow(measurement_img[i + j * rows].squeeze(-1).numpy(),
                                            cmap='gray', vmin=0, vmax=1)
                        else:
                            ax[i, j].imshow(measurement_img[i + j * rows].numpy())
            ...
            plt.savefig(os.path.join(
                args.save_path_ip, f"{args.problem}_measurement_batch{args.batch}_final.png")),
            plt.close(fig)

        if motion_kernel is not None:
            fig = plt.figure()
            plt.imshow(motion_kernel.cpu().numpy(), cmap='gray')
            plt.axis('off')
            plt.savefig(os.path.join(
                args.save_path_ip, f"{args.problem}_kernel_batch{args.batch}_final.png")),
            plt.close(fig)

        if ct_fbp_img is not None:
            if batch_size == 1:
                fig = plt.figure()
                plt.imshow(ct_fbp_img[0].numpy())
            elif batch_size == 2:
                fig, ax = plt.subplots(1, 2)
                ax[0].imshow(ct_fbp_img[0].numpy())
                ax[1].imshow(ct_fbp_img[1].numpy())
            else:
                fig, ax = plt.subplots(rows, cols, figsize=(20, 20))
                for i in range(rows):
                    for j in range(cols):
                        ax[i, j].imshow(ct_fbp_img[i + j * rows].numpy())
            ...
            plt.savefig(os.path.join(
                args.save_path_ip, f"{args.problem}_fbp_batch{args.batch}_final.png")),
            plt.close(fig)
    ...
    if args.eval_split == 'test':
        if ((args.batch < 8 and args.method == 'd_flow') or args.batch < 4):
            ...
            for i in range(batch_size):
                ...
                if measurement_img is not None:
                    fig = plt.figure()
                    plt.imshow(measurement_img[i].numpy())
                    plt.axis('off')
                    plt.savefig(os.path.join(
                        args.save_path_ip, f"{args.problem}_measurement_batch{args.batch}_im{i}.eps"),
                        bbox_inches='tight', pad_inches=0)
                    plt.close(fig)
...
def preprocess(img, args):
    if args.model == "rectified":
        import flowadmm.image_generation.datasets as datasets
        from flowadmm.image_generation.configs.rectified_flow.celeba_hq_pytorch_rf_gaussian import get_config as get_config_celebahq
        from flowadmm.image_generation.configs.rectified_flow.afhq_cat_pytorch_rf_gaussian import get_config as get_config_afhq_cat
        if args.dataset == "celebahq":
            config = get_config_celebahq()
...
def _display_noisy_image(noisy_img, args, H_adj):
    backprojection = postprocess(H_adj(noisy_img), args)
    if torch.is_complex(noisy_img) or args.problem in ['superresolution', 'superresolution_bicubic', 'ct']:
        return backprojection
    return postprocess(noisy_img.clone(), args)
...
def _normalize_for_display(img):
    ...
    return (img - mins) / (maxs - mins + 1e-8)
...
def _display_measurement_image(noisy_img, args):
    if torch.is_complex(noisy_img):
        meas = torch.fft.fftshift(torch.abs(noisy_img), dim=(-2, -1))
        meas = torch.log1p(meas)
        return _normalize_for_display(meas.float())

    if args.problem == 'ct':
        return _normalize_for_display(noisy_img.float())

    return None


def _ct_filtered_backprojection(noisy_img, dim_image):
    if noisy_img.ndim != 4:
        raise ValueError("Expected CT sinogram with shape [B, C, A, W]")

    device = noisy_img.device
    dtype = noisy_img.dtype
    _, _, num_angles, det_width = noisy_img.shape

    freqs = torch.fft.fftfreq(det_width, d=1.0, device=device, dtype=dtype)
    ramp = freqs.abs().view(1, 1, 1, det_width)
    sino_f = torch.fft.fft(noisy_img, dim=-1)
    filtered = torch.real(torch.fft.ifft(sino_f * ramp, dim=-1))

    angles = torch.linspace(
        0.0, np.pi, steps=num_angles + 1, device=device, dtype=dtype)[:-1]
    backproj = torch.zeros(
        noisy_img.shape[0], noisy_img.shape[1], dim_image, dim_image,
        device=device, dtype=dtype)
    scale = np.sqrt(dim_image)

    for i, angle in enumerate(angles):
        strip = filtered[:, :, i, :].unsqueeze(2).expand(-1, -1, dim_image, -1) / scale
        c = torch.cos(angle)
        s = torch.sin(angle)
        theta = torch.tensor([[c, s, 0.0], [-s, c, 0.0]],
                             device=device, dtype=dtype)
        theta = theta.unsqueeze(0).expand(noisy_img.shape[0], -1, -1)
        grid = F.affine_grid(theta, strip.size(), align_corners=False)
        backproj = backproj + F.grid_sample(
            strip, grid, mode='bilinear', padding_mode='zeros', align_corners=False)

    return backproj * (np.pi / (2.0 * max(num_angles, 1)))


def _ct_fbp_for_display(noisy_img, args):
    if args.problem != 'ct':
        return None
    fbp = _ct_filtered_backprojection(noisy_img.float(), int(args.dim_image))
    return postprocess(fbp, args)


def _motion_blur_kernel_for_display(args, device):
    if args.problem != 'motion_deblurring_FFT':
        return None

    length = max(1, int(getattr(args, 'motion_blur_length', 31)))
    angle_deg = float(getattr(args, 'motion_blur_angle_deg', 20.0))
    size = max(length, int(getattr(args, 'motion_blur_kernel_size', 61)))
    if size % 2 == 0:
        size += 1
    ...
    half = (length - 1) / 2.0
    num_points = max(2 * length, 1)

    xs = center + np.linspace(-half, half, num_points) * np.cos(theta)
    ys = center + np.linspace(-half, half, num_points) * np.sin(theta)
...
def _metric_batch_filename(metric_name, word, batch, suffix=''):
    return f'{metric_name}_{word}_batch{batch}{suffix}.txt'


def _metric_average_filename(metric_name, word, suffix=''):
    return f'{metric_name}_{word}_average{suffix}.txt'


def _compute_average_metric(args, metric_name, suffix=''):
    dict_metric = {}
    for word in ['rec', 'noisy']:
        metric_by_iteration = defaultdict(list)

        for batch in range(args.max_batch):
            filename = os.path.join(
                args.save_path_ip, _metric_batch_filename(metric_name, word, batch, suffix))

            with open(filename, 'r') as f:
                for line in f:
                    iteration, metric_value = map(float, line.strip().split())
                    metric_by_iteration[int(iteration)].append(metric_value)

        metric_averages = {
            iteration: np.mean(values)
            for iteration, values in metric_by_iteration.items()
        }

        avg_filename = os.path.join(
            args.save_path_ip, _metric_average_filename(metric_name, word, suffix)
        )

        with open(avg_filename, 'a') as f:
            for iteration, avg_value in sorted(metric_averages.items()):
                f.write(f'{iteration} {avg_value:.4f}\n')

        with open(avg_filename, 'r') as file:
            lines = file.readlines()
            metric_values = [float(line.split()[1]) for line in lines]
            dict_metric[word] = metric_values[-1]

    return dict_metric


def _write_final_metric_row(args, metric_name, dict_metric, extra_rec_metrics=None):
    final_filename = os.path.join(args.save_path, f'final_{metric_name}.txt')
    expected_header = [f'{metric_name}_rec', f'{metric_name}_noisy']
    if extra_rec_metrics is not None:
        expected_header.extend(list(extra_rec_metrics.keys()))
    expected_header.extend(list(args.dict_cfg_method.keys()))

    if os.path.exists(final_filename) and os.stat(final_filename).st_size > 0:
        with open(final_filename, 'r') as file:
            lines = file.readlines()
        current_header = lines[0].strip().split()
        if current_header != expected_header:
            upgraded_lines = [' '.join(expected_header) + '\n']
            num_param_cols = len(args.dict_cfg_method)
            num_extra_cols = len(expected_header) - 2 - num_param_cols
            for line in lines[1:]:
                tokens = line.strip().split()
                if not tokens:
                    continue
                if len(tokens) == 2 + num_param_cols:
                    tokens = tokens[:2] + (['nan'] * num_extra_cols) + tokens[2:]
                upgraded_lines.append(' '.join(tokens) + '\n')
            with open(final_filename, 'w') as file:
                file.writelines(upgraded_lines)

    with open(final_filename, 'a') as file:
        if os.stat(final_filename).st_size == 0:
            file.write(' '.join(expected_header))
            file.write('\n')

        file.write(f"{dict_metric['rec']} ")
        file.write(f"{dict_metric['noisy']} ")
        if extra_rec_metrics is not None:
            for value in extra_rec_metrics.values():
                file.write(f'{value} ')
...
def compute_psnr(clean_img, noisy_img, rec_img, args, H_adj, iter='final', suffix=''):

    # Ensure images are in the appropriate range and format for PSNR calculation
    clean_img = postprocess(clean_img.clone(), args)
    noisy_img = _display_noisy_image(noisy_img, args, H_adj)
    ...
    rec_filename = os.path.join(
        args.save_path_ip, _metric_batch_filename('psnr', 'rec', args.batch, suffix))
    ...
    noisy_filename = os.path.join(
        args.save_path_ip, _metric_batch_filename('psnr', 'noisy', args.batch, suffix))

    with open(noisy_filename, 'a') as file:
        file.write(f'{iter} {psnr_noisy}\n')


def compute_average_psnr(args, suffix='', append_final=True, extra_rec_metrics=None):
    dict_psnr = _compute_average_metric(args, 'psnr', suffix=suffix)
    if append_final:
        _write_final_metric_row(args, 'psnr', dict_psnr, extra_rec_metrics=extra_rec_metrics)
    return dict_psnr
...
def compute_lpips(clean_img, noisy_img, rec_img, args, H_adj, iter='final', suffix=''):

    DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    loss_fn_alex = _get_lpips_model()
    ...
    rec_filename = os.path.join(
        args.save_path_ip, _metric_batch_filename('lpips', 'rec', args.batch, suffix))
    ...
    noisy_filename = os.path.join(
        args.save_path_ip, _metric_batch_filename('lpips', 'noisy', args.batch, suffix))

    with open(noisy_filename, 'a') as file:
        file.write(f'{iter} {lpips_noisy}\n')


def compute_average_lpips(args, suffix='', append_final=True, extra_rec_metrics=None):
    dict_lpips = _compute_average_metric(args, 'lpips', suffix=suffix)
    if append_final:
        _write_final_metric_row(args, 'lpips', dict_lpips, extra_rec_metrics=extra_rec_metrics)
    return dict_lpips


def compute_ssim(clean_img, noisy_img, rec_img, args, H_adj, iter='final', suffix=''):
    # Ensure images are in the appropriate range and format for SSIM calculation
    clean_img = postprocess(clean_img.clone(), args).cpu()
    noisy_img = _display_noisy_image(noisy_img, args, H_adj).cpu()
    ...
    rec_filename = os.path.join(
        args.save_path_ip, _metric_batch_filename('ssim', 'rec', args.batch, suffix))
    ...
    noisy_filename = os.path.join(
        args.save_path_ip, _metric_batch_filename('ssim', 'noisy', args.batch, suffix))

    with open(noisy_filename, 'a') as file:
        file.write(f'{iter} {ssim_noisy}\n')


def compute_average_ssim(args, suffix='', append_final=True, extra_rec_metrics=None):
    dict_ssim = _compute_average_metric(args, 'ssim', suffix=suffix)
    if append_final:
        _write_final_metric_row(args, 'ssim', dict_ssim, extra_rec_metrics=extra_rec_metrics)
    return dict_ssim
