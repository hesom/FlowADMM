...
class PNP_FLOW(object):
    ...
    def grad_datafit(self, x, y, H, H_adj):
        ...
        elif self.args.noise_type in ('laplace', 'impulse'):
            # L1 subgradient: sign(Ax - y)
    ...
    def solve_ip(self, test_loader, degradation, sigma_noise, H_funcs=None):
        ...
        for batch in range(self.args.max_batch):
            ...
            if self.args.noise_type == 'gaussian':
                ...
                noisy_img += torch.distributions.laplace.Laplace(
                    torch.zeros_like(noisy_img), sigma_noise * torch.ones_like(noisy_img)).sample()
            elif self.args.noise_type == 'impulse':
                p = sigma_noise
                rand = torch.rand_like(noisy_img)
                noisy_img = torch.where(rand < p / 2,
                                        torch.zeros_like(noisy_img), noisy_img)
                noisy_img = torch.where(rand > 1 - p / 2,
                                        torch.ones_like(noisy_img), noisy_img)
            else:
                raise ValueError('Noise type not supported')
